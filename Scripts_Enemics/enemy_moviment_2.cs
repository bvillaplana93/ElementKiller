...
public class enemy_moviment_2 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        Debug.DrawRay(new Vector2(transform.position.x - 0.4f, transform.position.y - 2f), dirR * distL, Color.blue);

        hitR_ = Physics2D.Raycast(new Vector2(transform.position.x + 0.4f, transform.position.y), dirR, distR);
        Debug.DrawRay(new Vector2(transform.position.x + 0.4f, transform.position.y - 2f), dirR * distR, Color.blue);
